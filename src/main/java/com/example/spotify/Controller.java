...
public class Controller implements Initializable {
    ...
    @FXML
    private TableView<PlaylistClase> tabla_todas_playlist;

    @FXML
    private TableColumn<PlaylistClase, String> columna_playlisttodas;

    ObservableList<PlaylistClase> playlisttodasl;
    @FXML
    private TableView<PlaylistClase> tabla_playlist_patrocinada;

    @FXML
    private TableColumn<PlaylistClase, String> columna_playlist_patrocinada;

    ObservableList<PlaylistClase> playlistpatl;
    @FXML
            ...
            case "explora":
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        int bn = rs.getInt("id");
                        PlaylistClase a = new PlaylistClase(titu,bn);
                        playlisttodasl.add(a);
                    }
                    tabla_todas_playlist.setItems(playlisttodasl);
                ...
                try {
                    Statement stmt = Base_Datos.conexion().createStatement();
                    ResultSet rs = stmt.executeQuery("SELECT playlist.* FROM playlist" +
                            " INNER JOIN patrocinada p ON playlist.id = p.playlist_id");
// Procesa los resultados
                    while (rs.next()) {
                        String titua = rs.getString("titulo");
                        PlaylistClase o = new PlaylistClase(titua);
                        playlistpatl.add(o);
                    }
                    tabla_playlist_patrocinada.setItems(playlistpatl);
                    Base_Datos.conexion().close();
                } catch (SQLException e) {
                    e.printStackTrace();
                }
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        if (tit.equals(nombre_cancionrep.getText())) {
                            Image img = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/Imagenes/ic_lover.png")));
                            corazon.setImage(img);
                            pe = 0;
                            break;
                        } else {
                            Image img = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/Imagenes/ic_love.png")));
                            corazon.setImage(img);
                            pe = 1;
                        }
                    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        playlisttodasl = FXCollections.observableArrayList();
        columna_playlisttodas.setCellValueFactory(new PropertyValueFactory<>("nompl"));
        playlistpatl = FXCollections.observableArrayList();
   ...
   // PANEL DE AÑADIR AMIGOS//
    ...
    void seguir(ActionEvent event) {
        if (texto_usuario.getText().equals(nombre_usuario.getText())){
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Información");
            alert.setContentText("No te puedes seguir a ti mismo");
            alert.show();
        }else {
            try {
                Statement stmt = Base_Datos.conexion().createStatement();
                ResultSet rs = stmt.executeQuery("SELECT * FROM sigue_usuario WHERE usuario_id = (SELECT id FROM usuario WHERE username LIKE '" + nombre_usuario.getText() + "') AND usuario_seguido_id = (SELECT id FROM usuario WHERE username LIKE '" + texto_usuario.getText() + "')");
                if (rs.next()) {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Información");
...
//BOTONES DE IDIOMAS//
        ...
        if (titulo_playlist.getText().isEmpty()) {
            ...
            try {
                ...
                if (rs.next()) {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Información");
                    alert.setContentText("Esa playlist ya existe y la creastes");
                    alert.show();
                }else{
                    stmt.executeUpdate("INSERT INTO playlist(titulo,numero_canciones,fecha_creacion,usuario_id)" +
                            "VALUES('" + titulo_playlist.getText() + "',0,'" + fecha + "',(SELECT id FROM usuario WHERE username LIKE '" + nombre_usuario.getText() +"'))");
                    stmt.executeUpdate("INSERT INTO sigue_playlist(usuario_id,playlist_id)" +
                            "VALUES((SELECT id FROM usuario WHERE username LIKE '" + nombre_usuario.getText() + "'),(SELECT MAX(id) FROM playlist WHERE titulo LIKE '" + titulo_playlist.getText() + "'))");
                    panel_playlist.setVisible(false);
    ...
    void botonparaplaylist(ActionEvent event) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int ty = rs.getInt("id");
                PlaylistClase j = new PlaylistClase(t,ty);
                playlistl.add(j);
            }
    ...
    void seguir_art(ActionEvent event) {
        try {
            Statement stmt = Base_Datos.conexion().createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM sigue_artista WHERE usuario_id = (SELECT id FROM usuario WHERE username LIKE '" + nombre_usuario.getText() + "') AND artista_id = (SELECT id FROM artista WHERE nombre LIKE '" + texto_artista_si.getText() + "')");
        if (rs.next()){
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Información");
            alert.setContentText("Ese artista ya lo sigues");
            alert.show();
        }else {
...
// Procesa los resultados
    ...
    void dejar_de_seguir_playlist(ActionEvent event) {
        ...
        try {
            Statement stmt = Base_Datos.conexion().createStatement();
            stmt.executeUpdate("DELETE FROM sigue_playlist WHERE usuario_id = (SELECT id FROM usuario WHERE username LIKE '" + nombre_usuario.getText() + "') AND playlist_id = '" + ali.getId() + "'");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    void seguir_playlist(ActionEvent event) {
        ...
        try {
            Statement stmt = Base_Datos.conexion().createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM sigue_playlist WHERE usuario_id = (SELECT id FROM usuario WHERE username LIKE '" + nombre_usuario.getText() + "') AND playlist_id ='" + ali.getId() + "'");
            if (rs.next()) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Información");
                alert.setContentText("Esa playlist ya la sigues");
                alert.show();
            }else {
                System.out.println(ali.getId());
                stmt.executeUpdate("INSERT INTO sigue_playlist(usuario_id,playlist_id)" +
                        "VALUES((SELECT id FROM usuario WHERE username LIKE '" + nombre_usuario.getText() + "'),'" + ali.getId() + "')");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    void entrar_playlist(ActionEvent event) {
        ...
        canciones.clear();
        titulo_album.setText(artia.getNompl());
        try {
            Statement stmt = Base_Datos.conexion().createStatement();
            ResultSet rs = stmt.executeQuery("SELECT cancion.* FROM cancion " +
                    "INNER JOIN anyade_cancion_playlist a ON cancion.id = a.cancion_id INNER JOIN playlist p ON p.id = a.playlist_id WHERE p.titulo LIKE'" + titulo_album.getText() + "'");
