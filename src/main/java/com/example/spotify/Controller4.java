...
public class Controller4 {
    ...
    void free(ActionEvent event) {
        ...
        try {
            Statement stmt1 = Base_Datos.conexion().createStatement();
            ResultSet rs1 = stmt1.executeQuery("SELECT usuario.* FROM usuario WHERE username LIKE'" + usuario_intro.getText() + "'");
            if (rs1.next()) {
                ...
                if (usuarioi.isEmpty() || passi.isEmpty() || emaili.isEmpty() || generoi.isEmpty() || paisi.isEmpty() || codigoi.isEmpty()) {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Información");
                    alert.setContentText("Falta un campo por rellenar");
                    alert.show();
                } else {
                    try {
                    ...
                    try {
                        ...
                        if (rse.next()) {
                            ...
                            stmt.executeUpdate("INSERT INTO suscripcion(fecha_inicio,fecha_fin,premium_usuario_id)" +
                                    "VALUES('" + fecha_a + "','" + fecha_n + "','" + id_usuario + "')");
                        }
                        Base_Datos.conexion().close();
